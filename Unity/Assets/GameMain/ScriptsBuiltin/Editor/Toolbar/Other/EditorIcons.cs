...
namespace GameMain.Editor
{
    public class EditorIcons : EditorWindow
    {
        ...
        private static bool sViewBigIcons = true;

        private static bool sDarkPreview = true;

        private Vector2 mScroll;

        private int mButtonSize = 70;
        ...
        private bool DoSearch => !string.IsNullOrWhiteSpace(mSearch) && mSearch != "";
        ...
        private void SaveAllIcons()
        {
            ...
            try
            {
                foreach (var icon in ico_list)
                {
                    var split = icon.Split('/').Last();
                    var tex = EditorGUIUtility.IconContent(icon).image as Texture2D;
        ...
        {
            var allIcons = ico_list.Where(x => GetIcon(x) != null);
            var unique = new List<string>();

            foreach (var x in Resources.FindObjectsOfTypeAll<Texture2D>())
            {
                var icoContent = GetIcon(x.name);
        ...
        private void OnGUI()
        {
            ...
            using (new GUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                sViewBigIcons = GUILayout.SelectionGrid(sViewBigIcons ? 1 : 0, new string[] { "Small", "Big" }, 2, EditorStyles.toolbarButton) == 1;
            ...
            using (var scope = new GUILayout.ScrollViewScope(mScroll))
            {
                GUILayout.Space(10);

                mScroll = scope.scrollPosition;

                mButtonSize = sViewBigIcons ? 70 : 40;

                // scrollbar_width = ~ 12.5
                var renderWidth = (Screen.width / ppp - 13f);
                var gridW = Mathf.FloorToInt(renderWidth / mButtonSize);
                var marginLeft = (renderWidth - mButtonSize * gridW) / 2;
                ...
                if (DoSearch)
                    iconList = sIconContentListAll.Where(x => x.tooltip.ToLower()
                        .Contains(mSearch.ToLower())).ToList();
                else iconList = sViewBigIcons ? sIconContentListBig : sIconContentListSmall;
                ...
                while (index < iconList.Count)
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        for (var i = 0; i < gridW; ++i)
                        {
                            ...
                            if (GUILayout.Button(icon, sIconButtonStyle, GUILayout.Width(mButtonSize), GUILayout.Height(mButtonSize)))
                            {
            ...
            if (sIconSelected == null) return;

            GUILayout.FlexibleSpace();

            using (new GUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.MaxHeight(sViewBigIcons ? 140 : 120)))
            {
                using (new GUILayout.VerticalScope(GUILayout.Width(130)))
                {
                    GUILayout.Space(2);

                    GUILayout.Button(sIconSelected, sDarkPreview ? sIconPreviewBlack : sIconPreviewWhite, GUILayout.Width(128), GUILayout.Height(sViewBigIcons ? 128 : 40));

                    GUILayout.Space(5);

                    sDarkPreview = GUILayout.SelectionGrid(sDarkPreview ? 1 : 0, new string[] { "Light", "Dark" }, 2, EditorStyles.miniButton) == 1;
                ...
                {
                    var s = $"Size: {sIconSelected.image.width}x{sIconSelected.image.height}";
                    s += "\nIs Pro Skin Icon: " + (sIconSelected.tooltip.IndexOf("d_", StringComparison.Ordinal) == 0 ? "Yes" : "No");
                    s += $"\nTotal {sIconContentListAll.Count} icons";
                    GUILayout.Space(5);
                    EditorGUILayout.HelpBox(s, MessageType.None);
                    GUILayout.Space(5);
                    EditorGUILayout.TextField("EditorGUIUtility.IconContent(\"" + sIconSelected.tooltip + "\")");
                    GUILayout.Space(5);
                    if (GUILayout.Button("Copy to clipboard", EditorStyles.miniButton))
                        EditorGUIUtility.systemCopyBuffer = sIconSelected.tooltip;
                    if (GUILayout.Button("Save icon to file ...", EditorStyles.miniButton))
                        SaveIcon(sIconSelected.tooltip);
                }
        ...
        private static GUIContent sIconSelected;
        private static List<GUIContent> sIconContentListAll;
        private static List<GUIContent> sIconContentListSmall;
        private static List<GUIContent> sIconContentListBig;
        private static List<string> sIconMissingNames;
        private static GUIStyle sIconButtonStyle = null;
        private static GUIStyle sIconPreviewBlack = null;
        private static GUIStyle sIconPreviewWhite = null;
        ...
        {
            if (sIconContentListSmall != null) return;

            sIconButtonStyle = new GUIStyle(EditorStyles.miniButton);
            sIconButtonStyle.margin = new RectOffset(0, 0, 0, 0);
            sIconButtonStyle.fixedHeight = 0;

            sIconPreviewBlack = new GUIStyle(sIconButtonStyle);
            AllTheTextures(ref sIconPreviewBlack, Texture2DPixel(new Color(0.15f, 0.15f, 0.15f)));

            sIconPreviewWhite = new GUIStyle(sIconButtonStyle);
            AllTheTextures(ref sIconPreviewWhite, Texture2DPixel(new Color(0.85f, 0.85f, 0.85f)));

            sIconMissingNames = new List<string>();
            sIconContentListSmall = new List<GUIContent>();
            sIconContentListBig = new List<GUIContent>();
            sIconContentListAll = new List<GUIContent>();
            ...
            for (var i = 0; i < ico_list.Length; ++i)
            {
                ...
                if (ico == null)
                {
                    sIconMissingNames.Add(ico_list[i]);
                    continue;
                }

                ico.tooltip = ico_list[i];

                sIconContentListAll.Add(ico);

                if (!(ico.image.width <= 36 || ico.image.height <= 36))
                    sIconContentListBig.Add(ico);
                else sIconContentListSmall.Add(ico);
            }
        }
